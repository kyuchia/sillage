...
security add-generic-password -a "$USER" -s sillage-opensky-id \
    -T /usr/bin/security -U -w "$CID"
security add-generic-password -a "$USER" -s sillage-opensky-secret \
    -T /usr/bin/security -U -w "$CS"

echo "✅ stored sillage-opensky-id     (${#CID} chars, ...${CID: -4})"
echo "✅ stored sillage-opensky-secret (${#CS} chars, ...${CS: -4})"

# Verify the round-trip BEFORE destroying the only other copy.
BACK=$(security find-generic-password -w -s sillage-opensky-secret)
if [[ "$BACK" != "$CS" ]]; then
